//dijkstra without priority queue tle 31
#include <iostream>
#include <bits/stdc++.h>
#include <math.h>
#include <string.h>
...
int main() {
    ...
    fill(pathLength,pathLength+endV+1,ll_int_max);//from-to-value

    //init
    Parent[startV]=-1;
    pathLength[startV]=0;//dont set it visited in advance
...
int findShortest_path_among_unvisited(){
    ...
    //shortest among all
    for(;i<=endV;i++){
        if(pathLength[i]<pathLength[shortest] && !pathStutus[i]){
            shortest=i;
        }
    }
    return shortest;
